...
_register("bec_phishing", "Business Email Compromise (BEC)",
    ...
    "suspicious inbox rule → persistent SendAs delegation. v5.1.14: each phase's "
    "field_overrides matches one STAR rule's s1ql — Phase 2 (Okta) and Phase 3 (M365 admin "
    "consent) fire vendor-shipped rules; Phase 1 (Proofpoint), Phase 4 (M365 inbox rule) and "
    "Phase 5 (M365 mailbox delegation) fire API-Genie custom rules that key off the fields the "
    "tenant's OCSF collector preserves (the shipped rules for those three key off arrays the "
    "collector drops). NOTE: run this scenario for >=20 minutes so each M365 phase window "
    "overlaps at least one collector poll (M365 polls every 4 min); shorter runs cause Phase 3/4 "
    "windows to fall between polls and emit no events.",
    [
        # ── Phase 1 ─────────────────────────────────────────────────────────
        # Target rule (v5.1.14): "[apigenie] Proofpoint Impostor Email Delivered"
        # — a CUSTOM STAR rule generated by API Genie. The tenant's OCSF collector
        # strips the Proofpoint TAP `threatsInfoMap`, `disposition`,
        # `quarantineFolder` and `messageParts` arrays during normalisation
        # (verified empirically: all four arrive as null in the data lake even
        # though apigenie emits them on the wire). The vendor-shipped
        # "Proofpoint Impostor Email Unblocked" rule keys off
        # `unmapped.threatsInfoMap`, so it can NEVER fire on this tenant. The
        # custom rule instead keys off the scalar confidence scores the
        # collector DOES preserve — `unmapped.impostorScore` and
        # `unmapped.phishScore` — which is exactly the BEC initial-access signal.
        {
            ...
            "target_rules": [
                {
                    "name": "[apigenie] Proofpoint Impostor Email Delivered",
                    "source": "proofpoint",
                    "severity": "High",
                    "mitre": "T1566.001",
                    "custom": True,
