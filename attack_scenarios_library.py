...
_register("bec_phishing", "Business Email Compromise (BEC)",
    ...
    "suspicious inbox rule → persistent SendAs delegation. v5.1.15: Phase 1 (Proofpoint), "
    "Phase 2 (Okta) and Phase 3 (M365 admin consent) fire vendor-shipped STAR rules; Phase 4 "
    "(M365 inbox rule) and Phase 5 (M365 mailbox delegation) fire API-Genie custom rules that "
    "key off the fields the tenant's OCSF collector preserves (the shipped rules for those two "
    "key off the Parameters array the collector drops). NOTE: run this scenario for >=20 minutes "
    "so each M365 phase window overlaps at least one collector poll (M365 polls every 4 min); "
    "shorter runs cause Phase 3/4 windows to fall between polls and emit no events.",
    [
        # ── Phase 1 ─────────────────────────────────────────────────────────
        ...
        # ── Phase 4 ─────────────────────────────────────────────────────────
        # Target rule (v5.1.15): "M365 Inbox Rule Created via Legacy
        ...
        # for legacy client protocols (POP3/IMAP4/EWS) which BEC tooling uses.
        {
            ...
            "target_rules": [
                {
                    "name": "M365 Inbox Rule Created via Legacy Client Protocol",
                    "source": "m365",
                    "severity": "Medium",
                    "mitre": "T1564.008",
                    "custom": True,
        ...
        # ── Phase 5 ─────────────────────────────────────────────────────────
        # Target rule (v5.1.15): "M365 Mailbox Delegation Granted via
        ...
        # (preserved by the collector) instead of `unmapped.Parameters` (dropped).
        {
            ...
            "target_rules": [
                {
                    "name": "M365 Mailbox Delegation Granted via Legacy Client Protocol",
                    "source": "m365",
                    "severity": "Medium",
                    "mitre": "T1098.002",
                    "custom": True,
