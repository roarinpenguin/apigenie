...
def test_azure_platform_catalog_ids_match_template_keys():
    """The Kafka publisher delegates to ``sources.azure_platform``; the
    14-entry catalog must stay in lockstep with the ``_AZURE_TEMPLATES``
    dict so disabling a catalog entry from the admin UI actually drops
    that template from the producer."""
    from sources import azure_platform

    cat_ids = {e["id"] for e in azure_platform.EVENT_CATALOG}
    tpl_ids = set(azure_platform._AZURE_TEMPLATES.keys())
    assert cat_ids == tpl_ids, (
        f"azure_platform catalog/template drift: "
        f"catalog-only={cat_ids - tpl_ids}, template-only={tpl_ids - cat_ids}"
    )


# ── Source-id aliasing (bindings UI labels ≠ Python module filenames) ────────


def test_canonical_source_id_resolves_known_aliases():
    """The bindings UI uses ``entra_id`` and ``defender`` (Microsoft's
    marketing names) but the source modules live under ``azure_ad`` and
    ``microsoft_defender``. ``canonical_source_id`` must bridge the gap."""
    from sources import canonical_source_id

    assert canonical_source_id("entra_id") == "azure_ad"
    assert canonical_source_id("defender") == "microsoft_defender"
    # Non-alias ids must round-trip untouched so the helper is safe to
    # apply unconditionally at every storage boundary.
    assert canonical_source_id("azure_ad") == "azure_ad"
    assert canonical_source_id("okta") == "okta"
    assert canonical_source_id("unknown_source_xyz") == "unknown_source_xyz"


def test_get_event_catalog_accepts_aliases():
    """``get_event_catalog`` must accept both the UI alias and the module
    name and return the same catalog for both."""
    from sources import get_event_catalog

    by_alias = get_event_catalog("entra_id")
    by_module = get_event_catalog("azure_ad")
    assert by_alias is not None, "entra_id alias must resolve to azure_ad catalog"
    assert by_alias is by_module, (
        "alias and canonical lookup must return the exact same list object"
    )

    by_alias_d = get_event_catalog("defender")
    by_module_d = get_event_catalog("microsoft_defender")
    assert by_alias_d is not None
    assert by_alias_d is by_module_d


def test_get_event_catalogs_emits_under_alias_and_canonical_id():
    """The bindings UI iterates ``admin.SOURCES`` (which uses ``entra_id``
    and ``defender``) and looks up ``_mixAwareSources[src]``. The catalogs
    dict must contain BOTH the alias and the module name so either lookup
    succeeds — without this, the Event Mix disclosure would silently
    disappear from those two source cards even though the modules are
    fully wired."""
    from sources import get_event_catalogs

    catalogs = get_event_catalogs()
    assert "entra_id" in catalogs, "entra_id alias missing from /event-mix/sources payload"
    assert "azure_ad" in catalogs
    assert catalogs["entra_id"] == catalogs["azure_ad"]
    assert "defender" in catalogs
    assert "microsoft_defender" in catalogs
    assert catalogs["defender"] == catalogs["microsoft_defender"]


def test_alias_save_applies_to_canonical_module(_isolate_data_root):
    """End-to-end: storing a mix under the UI alias (``entra_id``) must
    actually affect events generated by the canonical module (``azure_ad``).
    This is the contract the admin.py endpoints uphold via
    ``canonical_source_id`` before any storage call. We exercise it here
    by writing storage directly with the canonical id (simulating the
    canonicalized PUT) and verifying the source module honours it."""
    em = _isolate_data_root
    # Disable all 6 directoryAudit ids on azure_ad (the canonical name).
    # An admin PUT against /admin/api/source-event-mix/entra_id with the
    # same payload would persist under exactly this key thanks to
    # canonical_source_id().
    audit_ids = [
        "user_updated", "mfa_registered", "ca_blocked",
        "risky_user_confirmed_safe", "service_principal_added",
        "impossible_travel",
    ]
    em.set_mix("azure_ad", [
    ...
    # Every entry in the audit endpoint should now be a residual fall-back
    # (resolver's "all disabled → use defaults" guard) rather than a real
    # disable-confirmation. The relevant check is that the mix WAS picked
    # up — easiest signal: an override was persisted under the canonical id.
    assert em.get_mix("azure_ad", user_id=None) is not None, (
        "Mix persisted under canonical id 'azure_ad' must be retrievable"
    )
    # Sanity: the response still serialises (no crash from the resolver's
...
def test_azure_platform_resolver_actually_disables_event(_isolate_data_root):
    """Disabling ``risky_user_detected`` should drop every event with
    ``category == 'RiskyUsers'`` from the Kafka producer's output.
    RiskyUsers is unique to that single template so it's a clean
    marker for empirical proof."""
    em = _isolate_data_root
    _apply_disable_mix(em, "azure_platform", ["risky_user_detected"])
    from sources import azure_platform

    random.seed(42)
    hits = 0
    for _ in range(300):
        ev = azure_platform.generate_azure_event()
        if ev.get("category") == "RiskyUsers":
            hits += 1
    assert hits == 0
